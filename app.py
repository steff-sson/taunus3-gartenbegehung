...
def preview():
    if request.method == 'POST':
        ...
        csv_file = os.path.join(os.path.dirname(__file__), 'static', f'gartenbegehung-{jahr}.csv')
        ...
        pdf_name = f'Parzelle-{session.get(\"parzelle\", \"x\")}---{hours}-{minutes}-{seconds}.pdf'
...
def done():
    for key in ['details', 'datum', 'dach', 'dachbauten', 'unkraut', 'drittelung', 'parzelle', 'strom', 'jahr']:
